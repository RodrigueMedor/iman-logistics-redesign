export const VEHICLE_SALES_URL = 'https://tiny-kringle-175161.netlify.app/'

// Replace this temporary URL when the dedicated masterclass website is ready.
export const FREIGHT_DISPATCH_URL = 'https://example.com/freight-dispatch-masterclass'

// Replace this temporary URL when the dedicated broker masterclass website is ready.
export const FREIGHT_BROKER_URL = 'https://example.com/freight-broker-masterclass'

export const TRUCKING_SCHOOL_URL = 'https://iman-trucking-school-website.netlify.app/'
